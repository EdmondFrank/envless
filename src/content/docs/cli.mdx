...
envless get    KEY                        [--env=ENV] --confirm [--pass=TOKEN]
...
must be intentional. If `ENVLESS_PASS_TOKEN` is set in the env's
secrets, `--pass=<token>` is also required and must match.
...
| `--pass` | _(none)_ | Pass token. Required when `ENVLESS_PASS_TOKEN` is set in the env's secrets; the provided value must match. If `ENVLESS_PASS_TOKEN` is not set, this flag is ignored. |
...
Output (stdout mode): the raw tarball bytes go to stdout, nothing
else; status messages all go to stderr so the pipe stays clean.
...
- `2` — usage error (e.g. `--include-identity` without `--yes` in a
  non-TTY context).
- `64` — configuration error: no `.envless/identity.key` found in
  the current directory or any parent.
- `74` — IO error (stage, tar invocation, file copy).

See [Operations → Backup & restore](/envless/operations/#backup--restore)
for upload-destination guidance and scheduled-backup recipes.
...
| `get` | `{env, key, confirm:true, pass?}` | `{value: "..."}` | `confirm` must be exactly `true` (bool) or `"true"` (string). `pass` required when `ENVLESS_PASS_TOKEN` is set in the env's secrets. |
...
| `1` | envless dispatcher | unhandled error (missing arg, parse failure, file not found, sops error, `get` without `--confirm`, pass token mismatch, etc.) |
...
- **`envless get`** — `0` and prints the value on success. `1` if
  ...
  not found in env "Y"`). `1` if `ENVLESS_PASS_TOKEN` is set but
  `--pass` is missing (stderr: `this env requires a pass token ...`)
  or the provided token doesn't match (stderr: `pass token
  mismatch`). `1` if `sops decrypt` errors.
...
- **`envless migrate`** — `0` on success (and on `--keep`). `1` if
  ...
  encrypt step fails.
- **`envless backup`** — `0` on success. `1` if the user cancels the
  `--include-identity` prompt. `2` for usage errors, including
  `--include-identity` in a non-TTY context without `--yes`. `64` if
  no `.envless/identity.key` is found by walking up from cwd. `74`
  for IO failures during staging or tar invocation.
...
| `backup` | [`zig/src/cli/backup.zig`][gh-backup] + [`zig/src/backup.zig`][gh-backup-mod] |
...
[gh-backup]: https://github.com/biliboss/envless/blob/main/zig/src/cli/backup.zig
[gh-backup-mod]: https://github.com/biliboss/envless/blob/main/zig/src/backup.zig
