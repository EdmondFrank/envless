...
- **`envless exec`** — **child exit code**, verbatim, when the child
  ...
  found — that is the OS exec failure, not an `envless` exit. The
  Go `os/exec` package surfaces this as `*exec.ExitError`, which
...
Any non-`ExitError` returned from the run path bubbles up to the
cobra default handler, which prints to stderr and exits `1`.
...
The `sops` call ([`sopswrap.go`][gh-sopswrap]) builds its env like
so:
