...
When the daemon is running, `envless exec` and `envless list` (MCP)
check the socket first. On a cache hit, the sops decrypt is skipped
entirely — latency drops from ~200 ms (sops spawn + decrypt) to
sub-millisecond (socket round-trip). On a cache miss, the daemon
decrypts, caches, and returns.

The MCP `get` tool uses the store directly (not the daemon) so the
full decrypted map is available for the `ENVLESS_PASS_TOKEN` safety
gate check. `get` is a rare, deliberate operation; the daemon cache
optimization is not critical here.
