...
- **Casual shoulder-surfing.** `envless list` prints keys but never
  values. `envless get` requires `--confirm`. When `ENVLESS_PASS_TOKEN`
  is set in an env's secrets, `--pass=<token>` is also required — a
  second knowledge factor that must match the stored token.
  `envless exec` writes secrets only into the child process's env
  array — never stdout.
...
- **A running process inspected via `ptrace`, `/proc/<pid>/environ`,
  ...
  child's memory is fair game. The optional [`envless daemon`](/envless/cli/#envless-daemon)
  widens this window: when installed, decrypted env stays in the
  daemon process's heap for up to 60 seconds per `(repo, env)` pair,
  so a ptrace-capable attacker has a longer reach. Daemon mode is an
  **explicit opt-in** (`envless daemon install`) — the default install
  has no daemon and no resident plaintext outside the lifetime of a
  single `sops decrypt`. See [Operations → Daemon mode](/envless/operations/#daemon-mode)
...
- **Side channels in your editor / shell history.** `envless get
  --confirm` prints plaintext (and `--pass=<token>` when
  `ENVLESS_PASS_TOKEN` is set). So does `echo $SECRET`. If you type
...
- **Supply-chain attacks on the Zig stdlib or vendored deps.**
  `envless` ships zero runtime dependencies beyond Zig 0.16.0's
...
```bash
# If ENVLESS_PASS_TOKEN is set in an env, add --pass=<token> to each get.
...
- **Never paste `envless get FOO --confirm` output into a prompt.**
  ...
  provider's API and may be cached. (When `ENVLESS_PASS_TOKEN` is set,
  the command also needs `--pass=<token>` — but the output is the same
  plaintext value, so the same caution applies.)
...
Zero runtime dependencies beyond Zig 0.16.0's stdlib + libc. The
...
- Pins Zig 0.16.0 via `goto-bus-stop/setup-zig@v2`.
