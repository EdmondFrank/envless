...
The tap lives in the main repo (no separate `homebrew-envless` repo)
...
Download from
[GitHub Releases](https://github.com/biliboss/envless/releases). The
...
Building requires [Zig 0.16.0](https://ziglang.org/download/) — pinned
in `zig/.zigversion`.
...
shell-history leaks. If `ENVLESS_PASS_TOKEN` is set in the env's
secrets, `--pass=<token>` is also required.

```bash
envless get OPENAI_API_KEY --confirm
```

To enable the pass token gate, set `ENVLESS_PASS_TOKEN` in any env:

```bash
echo "my-secret-token" | envless set ENVLESS_PASS_TOKEN
envless get OPENAI_API_KEY --confirm --pass=my-secret-token
```
