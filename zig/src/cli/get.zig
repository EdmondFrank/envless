...
pub fn run(ctx: *root.Context, args: []const []const u8) !u8 {
    ...
    const confirm = try root.popBoolFlag(ctx.allocator, rest_after_env.items, "--confirm", &rest);
...
fn printHelp(ctx: *root.Context) !void {
    ...
    try w.interface.writeAll("  envless get KEY --confirm [--env=NAME]\n\n");
    ...
    try w.interface.writeAll("  1    sops decrypt failure or --confirm missing\n");
